...
import { likePost, deletePost, getPosts, getPostsBySearch } from '../../../actions/posts';
...
const Post = ({ post, setCurrentId }) => {
  ...
  const Likes = () =>{
    if(post.likes.length > 0){
      return post.likes.find(liked => liked === (user?.result?.googleId || user?.result?._id))
  ...
  const Edit = () =>{
    if(post?.creator === user?.result?.googleId || post?.creator === user?.result?._id){
      return <MoreHorizIcon fontSize="medium" />
    }
    return <></>
  }
  ...
  return (
    <Card className={classes.card} raised elevation={6}>
      ...
      <CardActions className={classes.cardActions}>
        ...
        <Button size="small" color="primary" onClick={() => {dispatch(deletePost(post._id)); dispatch(getPosts())}}><Delete /></Button>
      </CardActions>
